...
function getPersonById(data, callback) {
    let query = "SELECT * FROM peer where id = ?";
    db.query(query, [data.id], function (err, data, fields) {
        if (err) {
            callback(err, null);
        }
...
function setSig(data, callback) {
    ...
    var id = data.id;
    let query = "Update peer SET signature = ? where id = ?";
